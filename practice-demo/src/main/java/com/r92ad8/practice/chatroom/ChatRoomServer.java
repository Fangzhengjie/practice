package com.r92ad8.practice.chatroom;
...
public class ChatRoomServer {
    ...
    public ChatRoomServer() {
        ...
        frame.setIconImage(Toolkit.getDefaultToolkit().createImage(ChatRoomServer.class.getResource("qq.png")));
        ...
        chatRoomUserList = new JList(listModel);

        southPanel = new JPanel(new BorderLayout());
        southPanel.setBorder(new TitledBorder("写消息"));
        southPanel.add(txt_message, "Center");
        southPanel.add(btn_send, "East");
        leftPanel = new JScrollPane(chatRoomUserList);
            ...
            while (true) {// 不停的等待客户端的链接
                try {
                    ...
                    if (clients.size() == max) {// 如果已达人数上限
                        ...
                        ChatRoomUser ChatRoomUser = new ChatRoomUser(st.nextToken(), st.nextToken());
                        // 反馈连接成功信息
                        w.println("MAX@服务器：对不起，" + ChatRoomUser.getName()
                                + ChatRoomUser.getIp() + "，服务器在线人数已达上限，请稍后尝试连接！");
                        w.flush();
                        // 释放资源
                        r.close();
                        w.close();
                    ...
                    clients.add(client);
                    listModel.addElement(client.getChatRoomUser().getName());// 更新在线列表
                    contentArea.append(client.getChatRoomUser().getName()
                            + client.getChatRoomUser().getIp() + "上线!\r\n");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
    ...
    class ClientThread extends Thread {
        ...
        public ClientThread(Socket socket) {
            try {
                ...
                StringTokenizer st = new StringTokenizer(inf, "@");
                ChatRoomUser = new ChatRoomUser(st.nextToken(), st.nextToken());
                // 反馈连接成功信息
                writer.println(ChatRoomUser.getName() + ChatRoomUser.getIp() + "与服务器连接成功!");
                writer.flush();
                // 反馈当前在线用户信息
                if (clients.size() > 0) {
                    String temp = "";
                    for (int i = clients.size() - 1; i >= 0; i--) {
                        temp += (clients.get(i).getChatRoomUser().getName() + "/" + clients
                                .get(i).getChatRoomUser().getIp())
                                + "@";
                    }
                    writer.println("ChatRoomUserLIST@" + clients.size() + "@" + temp);
                    writer.flush();
                }
                // 向所有在线用户发送该用户上线命令
                for (int i = clients.size() - 1; i >= 0; i--) {
                    clients.get(i).getWriter().println(
                            "ADD@" + ChatRoomUser.getName() + ChatRoomUser.getIp());
                    clients.get(i).getWriter().flush();
                }
            } catch (IOException e) {
                e.printStackTrace();
        ...
        public void run() {// 不断接收客户端的消息，进行处理。
            ...
            while (true) {
                try {
                    ...
                    {
                        contentArea.append(this.getChatRoomUser().getName()
                                + this.getChatRoomUser().getIp() + "下线!\r\n");
                        // 断开连接释放资源
                        reader.close();
                        writer.close();
                        socket.close();
                        ...
                        for (int i = clients.size() - 1; i >= 0; i--) {
                            clients.get(i).getWriter().println(
                                    "DELETE@" + ChatRoomUser.getName());
                            clients.get(i).getWriter().flush();
                        }

                        listModel.removeElement(ChatRoomUser.getName());// 更新在线列表

                        // 删除此条客户端服务线程
                        for (int i = clients.size() - 1; i >= 0; i--) {
                            if (clients.get(i).getChatRoomUser() == ChatRoomUser) {
